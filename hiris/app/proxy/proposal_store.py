...
class ProposalStore:
    ...
    - archived → dopo 30 giorni totali → DELETE
    ...
    def save(self, proposal: dict) -> str:
    ...
    def apply(self, proposal_id: str) -> bool:
        with self._mu:
            cur = self._conn.execute(
                "UPDATE automation_proposals SET status='applied', applied_at=? WHERE id=? AND status='pending'",
    ...
    def reject(self, proposal_id: str) -> bool:
        with self._mu:
            cur = self._conn.execute(
                "UPDATE automation_proposals SET status='rejected', rejected_at=? WHERE id=? AND status='pending'",
                (self._now(), proposal_id),
            )
            self._conn.commit()
        return cur.rowcount > 0
    ...
    def _run_lifecycle(self) -> None:
        """pending > 7gg → archived. archived > 30gg totali → DELETE."""
        ...
        if archived or deleted:
            logger.info("ProposalStore lifecycle: archived=%d deleted=%d", archived, deleted)
