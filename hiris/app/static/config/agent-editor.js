  ...
  function mount(agentId) {
    ...
    ensureLegacy().then(function() {
      ...
      if (agentId && typeof openAgent === 'function') {
        ...
        if (typeof resetAgentForm === 'function') resetAgentForm();
        else {
      ...
      console.error('Failed to load legacy modules', e);
      var outlet = document.getElementById('route-outlet');
      outlet.innerHTML = '<div style="padding:24px;color:var(--err)"><h2>Errore caricamento editor</h2><p>' + e.message + '</p></div>';
    });
  }
