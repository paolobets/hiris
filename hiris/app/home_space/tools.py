...
VIEW_TOOL_DEF = {
    ...
    "description": (
        ...
        "e quando si puo', quel confronto e' GIA' nella stessa risposta: "
        "`avvio_home_assistant`, accanto a `mute_da`, senza bisogno di "
        "un'altra chiamata. E' lo stato di `sensor.uptime`, che porta «the "
        "date and time when Home Assistant was last started» (documentazione "
        "dell'integrazione Uptime: device class timestamp, scritto una "
        "volta all'avvio e fermo fino al riavvio successivo). Vicino a "
        "`mute_da` e' un riavvio; lontano e' un guasto; **in mezzo non si "
        "sceglie, si dice che non si sa**. E quando `avvio_home_assistant` "
        "non compare -- l'integrazione Uptime e' facoltativa e molte case "
        "non l'hanno -- non si conclude «allora e' un guasto»: si torna a "
        "non affermare, perche' una regola che funziona solo dove il "
        "confronto c'e' mente dappertutto dove non c'e'. Se un "
