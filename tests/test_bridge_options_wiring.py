...
def test_bridge_options():
    ...
    2.4.0: le opzioni del ponte si sono SPOSTATE sotto la chiave madre
    `ponte:` -- e' l'unico raggruppamento che il Supervisor rende a schermo --
    e i due interruttori (`bridge_enabled` e `chat_via_subscription`) sono
    stati FUSI in `ponte.attivo`, perche' non erano due decisioni. Cambia il
    percorso e il nome dell'opzione, non la variabile d'ambiente: gli assert su
    `run.sh` qui sotto restano identici perche' `BRIDGE_ENABLED` sopravvive e
    adesso e' l'unica. Questo test si adegua perche' il soggetto si e' spostato
    e poi dimezzato, non perche' dava fastidio: l'invariante che pinna -- le
    opzioni del ponte esistono in tutti e quattro i posti, `bridge_fallback` in
    nessuno -- e' intatta, e in piu' adesso pinna che stiano SOTTO la sezione e
    non anche in cima."""
    # Test 1: config.yaml has bridge options and schema
    cfg = yaml.safe_load((BASE / "config.yaml").read_text(encoding="utf-8"))
    opzioni, schema = cfg["options"]["ponte"], cfg["schema"]["ponte"]
    assert "attivo" in opzioni, "l'interruttore del ponte non e' in config.yaml options.ponte"
    ...
    assert "attivo" not in cfg["options"], "l'interruttore deve stare SOTTO `ponte:`, non anche in cima"

    assert "attivo" in schema, "l'interruttore del ponte non e' in config.yaml schema.ponte"
    ...
    assert "attivo:" in it_content, "l'interruttore del ponte non e' in it.yaml"
    assert "attivo:" in en_content, "l'interruttore del ponte non e' in en.yaml"
