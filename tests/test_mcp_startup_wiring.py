...
def test_build_internal_mcp_server_binds_loopback(monkeypatch):
    ...
    # I-2 partial (Plan 2B final review, fast-follow): build_internal_mcp_server
    # now also returns the McpGuard so it's reachable outside this function
    # (see server.py's _on_startup, which stores it on app["mcp_guard"]).
    client, config, guard = build_internal_mcp_server(hiris_base_url="http://127.0.0.1:8099")
