...
preme nessuno», 26/08/2026): `build_companions` tornava solo `mappa`.
...
from hiris.app.server import build_companions
...
async def test_i_comprimari_arrivano_da_legami():
    ...
    mappa, falliti = await build_companions(ha, ["climate.camera_t"])
...
async def test_si_chiede_sempre_il_tipo_giusto_a_home_assistant():
    """Il Critical vero: `build_companions` chiedeva `"entita"` (la
...
async def test_un_guasto_di_sistema_non_si_chiede_a_legami():
    ...
    ha = _ClienteLegami({})
    mappa, falliti = await build_companions(ha, ["problema:sonos.x", "integrazione:abc"])
    assert mappa == {}
    assert falliti == 0
    assert ha.chiesti == []
...
async def test_un_guasto_di_legami_logga_col_prefisso_cervello(caplog):
    ...
    **Correzione del residuo 2 (grilletto-brief.md, appendice):** l'assert
    ...
    with caplog.at_level(logging.WARNING, logger="hiris.app.server"):
        await build_companions(ha, ["climate.camera_t"])
...
async def test_si_chiede_una_volta_sola_per_soggetto():
    ...
    ha = _ClienteLegami({})
    await build_companions(ha, ["climate.a", "climate.a", "climate.b"])
...
async def test_falliti_conta_solo_i_soggetti_rotti_non_tutti():
    ...
    -- e' esattamente il numero che `reaggregate_last_two_days` legge
