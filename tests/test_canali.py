...
Oggi quattro portatori -- il ponte, il gateway MCP su un'altra macchina, il
proxy di Retro Panel e la porta di sviluppo -- presentano **lo stesso segreto
condiviso**. Un segreto condiviso non e' un'identita': e' una parola d'ordine, e
chi la sente una volta e' tutti. Compromessa una qualunque delle quattro strade
l'unica mossa e' cambiare il token e romperle tutte insieme, e nessun registro
puo' dire QUALE integrazione ha chiamato.
...
def _registrate(pubblica, *, nome="sviluppo", ruolo="lettore"):
    return {nome: {"ruolo": ruolo, "chiave": pubblica}}


def _firma(privata, *, canale="sviluppo", metodo="GET",
           percorso="/api/entities", momento=None, unico="u-1", corpo=b""):
    momento = time.time() if momento is None else momento
    materia = canali.materia_firmata(metodo, percorso, momento, unico, corpo)
    return {"canale": canale, "momento": momento, "unico": unico,
            "firma": base64.b64encode(privata.sign(materia)).decode("ascii"),
            "metodo": metodo, "percorso": percorso, "corpo": corpo}


def _verifica(registrate, visti=None, adesso=None, **richiesta):
    return canali.riconosci(
        registrate=registrate, visti={} if visti is None else visti,
...
def test_una_firma_valida_da_il_canale_e_il_suo_RUOLO():
    ...
    chiamato -- cosa che oggi non puo' -- e con quale perimetro.

    Mutazione: accettare senza verificare -- rossa."""
    privata, pubblica = _coppia()

    esito, motivo = _verifica(_registrate(pubblica), **_firma(privata))

    assert motivo is None, motivo
    assert esito["canale"] == "sviluppo"
...
def test_la_firma_di_un_ALTRA_chiave_non_passa():
    ...
    esito, motivo = _verifica(_registrate(pubblica), **_firma(altra))
...
def test_il_CORPO_e_dentro_la_firma():
    ...
    esito, _ = _verifica(_registrate(pubblica, ruolo="utente"), **richiesta)
...
def test_anche_il_PERCORSO_e_dentro_la_firma():
    ...
    esito, _ = _verifica(_registrate(pubblica), **richiesta)
...
def test_una_richiesta_VECCHIA_non_passa():
    ...
    esito, motivo = _verifica(
        _registrate(pubblica), adesso=adesso,
...
def test_la_STESSA_richiesta_non_si_RIGIOCA():
    ...
    primo, _ = _verifica(_registrate(pubblica), visti=visti, **richiesta)
    secondo, motivo = _verifica(_registrate(pubblica), visti=visti, **richiesta)
...
def test_i_valori_gia_visti_non_crescono_per_sempre():
    ...
    _verifica(_registrate(pubblica), visti=visti, adesso=adesso,
              **_firma(privata, momento=adesso))

    assert len(visti) < 10, f"i valori scaduti sono rimasti: {len(visti)}"


# --- la registrazione: tre rifiuti, ognuno dice quale manca ------------------

def test_un_canale_NON_DICHIARATO_non_passa_nemmeno_con_la_firma_giusta():
    """Il ruolo e la chiave stanno nelle opzioni; **che il canale esista** sta
    nel codice. Un nome che nessuno ha dichiarato -- e per cui quindi nessuno
    ha deciso niente -- e' rifiutato anche con una firma perfetta.

    Mutazione ESEGUITA: fidarsi del solo elenco delle registrazioni -- rossa."""
    privata, pubblica = _coppia()

    esito, motivo = _verifica(
        _registrate(pubblica, nome="inventato"),
        **_firma(privata, canale="inventato"))

    assert esito is None
    assert "dichiar" in motivo.lower()


def test_un_canale_dichiarato_SENZA_registrazione_non_passa():
    """Dichiarare un canale nel codice non gli da' accesso: serve che qualcuno
    incolli la sua chiave nelle opzioni.

    Mutazione: ripiegare su un elenco vuoto e accettare -- rossa."""
    privata, _ = _coppia()

    esito, motivo = _verifica({}, **_firma(privata))

    assert esito is None
    assert motivo


def test_una_registrazione_senza_RUOLO_non_passa():
    """Una chiave senza ruolo erediterebbe qualcosa in silenzio, e quel
    qualcosa sarebbe deciso dal codice invece che dal proprietario.

    Mutazione ESEGUITA: ripiegare su «utente» quando il ruolo manca -- rossa."""
    privata, pubblica = _coppia()

    esito, motivo = _verifica({"sviluppo": {"chiave": pubblica}},
                              **_firma(privata))
...
def test_un_ruolo_SCONOSCIUTO_non_passa():
    ...
    esito, motivo = _verifica(_registrate(pubblica, ruolo="capo"),
                              **_firma(privata))
...
def test_una_firma_STORTA_non_fa_cadere_la_verifica():
    ...
    for guasta in ("", "non-base64!!", base64.b64encode(b"corta").decode()):
        esito, motivo = _verifica(
            _registrate(pubblica),
            canale="sviluppo", momento=time.time(), unico="u-x", firma=guasta, metodo="GET",
                percorso="/api/entities", corpo=b"")
        assert esito is None and motivo


def test_una_CHIAVE_storta_non_fa_cadere_la_verifica():
    """La chiave la incolla una persona nella pagina del Supervisor: prima o
    poi qualcuno incollera' male.

    Mutazione: costruire la chiave senza difendersi -- rossa."""
    privata, _ = _coppia()
    for guasta in ("", "non-base64!!", base64.b64encode(b"corta").decode()):
        esito, motivo = _verifica(_registrate(guasta), **_firma(privata))
        assert esito is None and motivo


# --- come si leggono le registrazioni dalle opzioni -------------------------

def test_le_registrazioni_si_leggono_una_per_riga():
    """La forma che il proprietario incolla: `nome · ruolo · chiave`.

    Mutazione: accettare una riga a due campi come valida -- rossa (una chiave
    finirebbe nel campo del ruolo)."""
    _, pubblica = _coppia()
    testo = f"sviluppo:lettore:{pubblica}\ngateway:utente:{pubblica}\n"

    lette = canali.leggi_registrazioni(testo)

    assert lette["sviluppo"] == {"ruolo": "lettore", "chiave": pubblica}
    assert lette["gateway"]["ruolo"] == "utente"


def test_una_riga_STORTA_si_salta_e_non_ferma_le_altre():
    """Una riga sbagliata non deve far perdere i canali scritti bene: sarebbe
    un errore di battitura che spegne un'integrazione che non c'entra.

    Mutazione: sollevare alla prima riga storta -- rossa."""
    _, pubblica = _coppia()
    testo = f"buono:utente:{pubblica}\nsenza-campi\n\n  \n"

    lette = canali.leggi_registrazioni(testo)

    assert set(lette) == {"buono"}
...
def test_ogni_canale_dichiarato_ha_una_SPECIE_e_una_ragione():
    """Un canale senza specie non si saprebbe se e' una macchina o un luogo, e
    sono due cose diverse nella cronaca. Uno senza ragione e' indistinguibile
    da una dimenticanza.

    Mutazione: aggiungere un canale senza `perche` -- rossa."""
    assert canali.CANALI, "nessun canale dichiarato"
    for nome, riga in canali.CANALI.items():
        assert riga["specie"] in ("integrazione", "luogo"), (
            f"«{nome}» ha una specie sconosciuta: {riga['specie']!r}")
        assert len(riga.get("perche") or "") > 20, (
            f"«{nome}» non dice perche' esiste")
