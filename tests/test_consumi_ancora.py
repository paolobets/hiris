...
def test_azzerare_porta_i_totali_a_zero_senza_cancellare_niente(archivio):
    archivio.registra("claude", "m", richieste=5, token_in=500, costo_usd=2.0,
                      costo_stato="misurato", adesso=MATTINA)

    archivio.sposta_ancora(POMERIGGIO)

    assert archivio.totali(da_ancora=True)["richieste"] == 0, (
...
def test_dopo_l_ancora_il_consumo_nuovo_si_conta(archivio):
    archivio.registra("claude", "m", richieste=5, costo_usd=2.0,
                      costo_stato="misurato", adesso=MATTINA)
    archivio.sposta_ancora(POMERIGGIO)
    archivio.registra("claude", "m", richieste=2, costo_usd=1.0,
                      costo_stato="misurato", adesso=POMERIGGIO)

    assert archivio.totali(da_ancora=True)["richieste"] == 2
    assert archivio.totali(da_ancora=True)["costo_usd"] == 1.0
    assert archivio.totali()["richieste"] == 7


def test_l_ancora_vale_anche_sui_giorni_successivi(archivio):
    archivio.registra("claude", "m", richieste=5, costo_stato="misurato", adesso=MATTINA)
    archivio.sposta_ancora(POMERIGGIO)
    archivio.registra("claude", "m", richieste=3, costo_stato="misurato", adesso=DOMANI)
    assert archivio.totali(da_ancora=True)["richieste"] == 3


def test_senza_ancora_da_ancora_e_da_sempre_coincidono(archivio):
    archivio.registra("claude", "m", richieste=4, costo_stato="misurato", adesso=MATTINA)
    assert archivio.totali(da_ancora=True) == archivio.totali()
    assert archivio.ancora() == 0.0


def test_l_ancora_si_ricorda_quando_e_stata_spostata(archivio):
    archivio.sposta_ancora(POMERIGGIO)
    assert archivio.ancora() == POMERIGGIO


def test_le_sezioni_da_ancora_non_mostrano_cio_che_e_prima(archivio):
    archivio.registra("claude", "m", richieste=5, costo_stato="misurato", adesso=MATTINA)
    archivio.sposta_ancora(POMERIGGIO)
    archivio.registra("openrouter", "x", richieste=1, costo_usd=0.5,
                      costo_stato="reale", adesso=POMERIGGIO)
...
def test_i_file_usage_di_prima_entrano_una_volta_sola(archivio, tmp_path):
    ...
    quanti = archivio.importa_legacy([str(vecchio)], adesso=POMERIGGIO)
    assert quanti == 1
    assert archivio.importa_legacy([str(vecchio)], adesso=POMERIGGIO) == 0, (
...
def test_il_provider_si_deduce_dal_nome_del_file(archivio, tmp_path):
    for nome, atteso in (("usage_openai.json", "openai"),
        ...
        archivio.importa_legacy([str(tmp_path / nome)], adesso=POMERIGGIO)
...
def test_un_file_illeggibile_si_dichiara_e_non_ferma_gli_altri(archivio, tmp_path):
    ...
    assert archivio.importa_legacy([str(rotto), str(buono)], adesso=POMERIGGIO) == 1
    assert archivio.totali()["richieste"] == 7
