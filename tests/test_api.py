...
async def test_chat_usa_il_system_prompt_delle_impostazioni(client):
    client.app["chat_settings"] = ChatSettings(
...
async def test_chat_senza_chatbot_id_usa_comunque_le_impostazioni(client):
    client.app["chat_settings"] = ChatSettings(system_prompt="Prompt di default HIRIS.")
    runner = client.app["claude_runner"]
...
async def test_chat_con_chatbot_id_sconosciuto_non_rompe_e_ignora_lid(client):
    ...
    client.app["chat_settings"] = ChatSettings(system_prompt="Fallback prompt.")
    runner = client.app["claude_runner"]
...
async def test_chat_con_chatbot_id_si_comporta_come_senza(aiohttp_client, tmp_path):
    ...
    # `dispatcher` e' un oggetto per-app (un'istanza di ToolDispatcher
...
async def test_chat_context_e_limitato_dai_giorni_di_conservazione(client):
    ...
    data_dir = client.app["data_dir"]
    client.app["chat_settings"] = ChatSettings(retention_days=5)
    runner = client.app["claude_runner"]
    runner.chat = AsyncMock(return_value="ok")
...
# calling either; its context comes from the nucleo (hiris/app/home_space/
# nucleo.py, via handlers_home_space.compose_briefing) -- see
# tests/test_chat_briefing.py for the tests that replace these.
