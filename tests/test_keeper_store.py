...
# prove sul filo stanno in `test_promesse_divise.py`; qui ne basta uno.
...
def test_una_promessa_creata_si_rilegge_con_la_stessa_forma(archivio):
    esito = archivio.create(_fai(), thread=PAOLO, now=ADESSO)
...
def test_una_promessa_non_valida_non_entra(archivio):
    esito = archivio.create(_fai(quando_ts=ADESSO - 1), thread=PAOLO, now=ADESSO)
...
def test_scadute_torna_solo_cio_che_e_in_attesa_e_gia_scaduto(archivio):
    presto = archivio.create(_fai(quando_ts=ADESSO + 10),
                            thread=PAOLO, now=ADESSO)["promessa"]["id"]
    tardi = archivio.create(_fai(quando_ts=ADESSO + 9999),
                            thread=PAOLO, now=ADESSO)["promessa"]["id"]
...
def test_prendi_riesce_una_volta_sola(archivio):
    ...
    ident = archivio.create(_fai(quando_ts=ADESSO + 10),
                            thread=PAOLO, now=ADESSO)["promessa"]["id"]
...
def test_disdire_una_promessa_in_attesa_riesce_e_una_conclusa_no(archivio):
    ident = archivio.create(_fai(),
                            thread=PAOLO, now=ADESSO)["promessa"]["id"]
    assert "errore" not in archivio.cancel(ident, thread=PAOLO, now=ADESSO + 1)
...
def test_una_promessa_presa_non_si_disdice(archivio):
    ...
    esito = archivio.cancel(ident, thread=PAOLO, now=ADESSO + 21)
    assert "errore" in esito
    assert archivio.read(ident)["stato"] == "in_corso"
...
def test_il_tetto_delle_in_sospeso_rifiuta_nominandolo(archivio):
    for _ in range(CEILING_IN_SOSPESO):
        assert "errore" not in archivio.create(_fai(), thread=PAOLO, now=ADESSO)
    esito = archivio.create(_fai(), thread=PAOLO, now=ADESSO)
    assert "errore" in esito
...
def test_elenca_in_sospeso_include_anche_in_corso(archivio):
    ...
    sospese = archivio.list(thread=PAOLO, solo_in_sospeso=True)
...
def test_lo_stato_in_corso_conta_nel_tetto_delle_in_sospeso(archivio):
    ...
    for _ in range(CEILING_IN_SOSPESO - 1):
        archivio.create(_fai(), thread=PAOLO, now=ADESSO)
    ultima = archivio.create(_fai(),
                            thread=PAOLO, now=ADESSO)["promessa"]["id"]
...
def test_le_concluse_vecchie_si_potano_alla_scrittura_le_in_sospeso_mai(archivio):
    ...
    in_sospeso = archivio.create(_fai(quando_ts=ADESSO + 20 * 86400),
                            thread=PAOLO, now=ADESSO)["promessa"]["id"]

    dopo = ADESSO + CONSERVAZIONE_S + 86400
    recente = archivio.create(_fai(quando_ts=dopo + 3600), thread=PAOLO, now=dopo)["promessa"]["id"]
...
def test_la_potatura_misura_l_eta_dalla_conclusione_non_dalla_nascita(archivio):
    ...
    ident = archivio.create(_fai(quando_ts=nata + 3600), thread=PAOLO, now=nata)["promessa"]["id"]
