...
- **Chi vede e chi tocca.** `agenda`, `cancel`, `GET/DELETE /api/agenda`, `POST /api/agenda/read`, `GET /api/executions/{id}` (se legato a una promessa) e il conteggio `agenda_unread` lavorano **solo sul filo di chi chiede**; un id di un altro filo risponde come uno inesistente. Il tetto delle 50 pendenti è per filo (`CEILING_IN_SOSPESO`), e accanto c'è quello della casa, 200 in totale (`HOUSE_CEILING_IN_SOSPESO`, `keeper/promise.py`): i fili li moltiplica chiunque arrivi da un ingresso diverso, e un tetto solo per filo lascerebbe il totale crescere senza fine — su disco (la scheda SD) e nel battito dell'orologio, che legge le scadute ogni 15 secondi.
- **Orfane.** Le promesse senza filo (nate prima) vanno al proprietario, con la stessa funzione dell'archivio (`AgendaStore.adopt_orphans`: tutte le orfane, una volta sola), da **due porte**:
  - la sua prima richiesta dal pannello, insieme alla cronologia (`chat_thread.adopt_if_owner`: persona che Home Assistant dice proprietaria, ingresso `pannello`);
  - il **risveglio** di un'orfana, se matura prima di quella richiesta (`keeper/sweeper.py::_adopt_orphan`, review finale della fetta): l'orologio chiede a Home Assistant chi è il proprietario (`soffitto.sole_owner`, stesso lettore e stesso campo `proprietario` di `is_owner`) e, se è **uno e uno solo**, le adotta al suo filo `persona:<id>` / `pannello` e prosegue con le regole di sempre — soffitto rivalutato, esito nel filo, push ai suoi telefoni. Senza, chi usa HIRIS dal giorno prima si vedrebbe chiudere `fallita` un `fai` che fino al giorno prima partiva.
  
  Se Home Assistant non risponde, o i proprietari non sono esattamente uno, l'orfana resta orfana e si chiude nel dubbio: un `fai` non agisce (`_ORPHAN_FAI_REASON`), l'esito di un `chiedi` resta solo nella pagina (`_ORPHAN_REASON`). La cronologia della chat non si adotta al risveglio: resta alla prima richiesta dal pannello.
...
e questa funzione scarta: `_slugify("ß")` dà `"unknown"` (il caso limite di HA per un testo che si
riduce al niente), `_slugify("Straße")` dà `"strae"`. Non è un buco: un candidato del genere non risulta
...
- Un esito che arriva più di 2 ore dopo l'ultimo messaggio apre una sessione sua (la finestra di sessione della chat, `chat_store.SESSION_GAP_HOURS`). Se la persona risponde dopo altre 2 ore, la sua risposta apre un'altra sessione ancora, e il modello vede l'esito solo nel riassunto delle sessioni precedenti, non per intero. Limite accettato: rincorrerlo vorrebbe dire una regola di sessione diversa per gli esiti.
