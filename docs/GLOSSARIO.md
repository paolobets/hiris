...
(`azione/construction/advisor.py:20-32`, la struttura con `innesco`/`passi`/`stati`/`parametri`
...
- **`spazio` dentro `memory/`** (Task 5, prima review): `spazio (memory)` → `slot` e' il concetto
  di cache (`lookup_cache.py`, «a quale chiamante appartiene una voce»). Ma `spazio_precedente`,
...
`tests/js/dashboard-knowledge.test.mjs:44`, perche' la scansione elencava le estensioni a mano
...
**Un caso dichiarato, come la regola pretende.** `provider_name` vive ora in `backends/openai_compat_runner.py` (l'attributo del runner, gia' `provider_nome`) e in sei siti di `backends/embeddings.py`, dove era gia' inglese: **stesso concetto, stesso strato, stesso pacchetto** -- il nome con cui il prodotto chiama un provider di modelli. Non e' l'omonimia a due altezze di `logbook`, e' semplicemente la stessa cosa chiamata uguale in due file vicini, che e' cio' che un lettore si aspetta. Sarebbe stata da vietare il contrario: coniare un secondo nome per non ripetersi.
...
chiusi insieme.** E' gia' successo tre volte -- `carattere` (lotto 8) su `memory/resolver.py`,
`richiesto` (lotto 9) su `azione/construction/advisor.py`, `definizione` (lotto 10) su
`action/verification.py` -- e ogni volta l'unico contenimento e' stato
...
- **riga nuda** -- si applica ovunque: 21 identificatori di `hiris/app` verrebbero riscritti
  (`memory/resolver.py::fine`, `server.py::attuale`, `azione/construction/composer.py::numero`...) e
...
| comprimari | per un soggetto, le altre cose della casa -- entita', automazioni, scene, script -- che agiscono su di lui o lo misurano. **Dal 15/09/2026 vive solo nelle promesse** (`server.py::_COMPANION_TYPES`, i tipi a cui una promessa puo' legarsi): l'uso dell'osservatore -- «lette una sola volta per l'intera giornata e riusate per ogni fatto compiuto che ne nasce» -- e' uscito con lo strato degli oggetti, cancellato dopo averne misurato la resa: **zero comprimari su 200 oggetti** | companions | ~ parziale |
...
| impronta | il riassunto breve e stabile dell'insieme dei giudizi con cui un resoconto e' stato scritto, conservato accanto al resoconto stesso: due impronte diverse dicono che il giudizio e' cambiato dopo, ed e' il solo modo per sapere quali giorni vanno rifatti senza rifarli tutti. **Inglese gia' deciso** fra «Le parole ordinarie» (`fingerprint`) e applicato in `mind/facts.py`: questa riga aggiunge la funzione, non un nome nuovo | fingerprint | ✓ arriva |
...
| istantanea | tutti i giudizi in vigore in un dato momento, letti una volta sola e tenuti fermi per tutto il lavoro che li usa -- cosi' che un giorno di cronaca nasca da un insieme di giudizi coerente, e una correzione arrivata a meta' non lo spacchi in due; chi scrive un giudizio la ricostruisce e sostituisce quella vecchia, invece di modificarla sotto i piedi di chi legge. **Inglese gia' deciso** fra «Le parole ordinarie» (`snapshot`) e applicato in `mind/judgments.py`: questa riga aggiunge la funzione, non un nome nuovo | snapshot | ✓ arriva |
...
| elenco lungo | la forma unica con cui una pagina mostra un elenco che non ci sta: un riassunto coi numeri, i pochi che contano col criterio con cui sono stati scelti, e il resto dietro un bottone che **costruisce le righe quando lo si preme** -- a blocchi, oltre il centinaio. Nasce misurando: la pagina dell'osservatore disegnava oltre cinquecento righe in una colonna sola, e chiuderle in un rivelatore non sarebbe bastato, perche' chiuso non vuol dire non costruito | long list | ~ parziale |
| impalcatura | il giudizio che dice se un'integrazione e' Home Assistant che parla di se' -- il Supervisor, HACS, il frontend, il websocket -- invece che della casa: le sue condizioni di sistema restano nella cronaca ma non salgono in `primo piano`. **E' l'unico giudizio il cui soggetto non e' una cosa di casa**: non un tipo ne' un'entita', ma il nome breve dell'integrazione da cui viene la voce di registro. Si/no, e l'assenza vale no -- il contrario farebbe tacere la pagina su ogni integrazione che nessuno ha ancora giudicato, cioe' proprio quando una cosa si rompe per la prima volta | scaffolding | ~ parziale |
...
| primo piano | le poche righe di un giorno che chi abita la casa deve sapere senza andarle a cercare, scelte quando la pagina legge e mai salvate -- una condizione di sistema che Home Assistant ha segnalato, oppure un fatto di un'entita' il cui tipo dice che va saputo subito. **Non e' `notevole`**, che risponde a un'altra domanda («vale la pena raccontarlo dopo, in un riassunto?»): misurato sulla cronaca vera del 17/09/2026, `notevole` marcherebbe 57 voci su 75 e il primo piano ne porta 6. La parola «banda», che la spec usa in prosa, era gia' presa in `watcher-giorno.js` (la striscia dei comandi col selettore del giorno) | front page | ~ parziale |
...
| provenienza | **la meta' che riguardava l'osservatore e' uscita l'11/09/2026**: il pavimento non esiste piu' e la pagina dell'osservatore non mostra piu' nessuna `provenienza` -- al suo posto ci sono il `motivo` e l'`autore` di ogni decisione (`mind/scope.py`). Resta il senso dell'energia: lo stesso principio classifica anche una direzione dell'energia (dichiarata dalla dashboard dell'utente, o dedotta dall'integrazione quando la dichiarata tace) -- aggiunta dal Task 6, non ancora passata dalla prova del lettore nuovo. **Una seconda `provenienza`, in un altro modulo, prende lo stesso inglese e non e' la stessa cosa**: `decisione_modelli.provenance(provider_id, source, ...)` dice da dove viene l'ELENCO dei modelli che il pannello sta mostrando (letto adesso, oppure di riserva perche' la lettura e' fallita). E' omonimia FRA MODULI col concetto vicino ma non identico -- «da dove viene cio' che vedi» -- e rientra nel caso ammesso descritto sopra per `logbook`. La CHIAVE JSON `"provenienza"` del cervello resta italiana, e i due non si incontrano mai in una firma | provenance | ~ non provato |
...
| scrittura | i token che una chiamata ha fatto CREARE nella cache del provider, il lato opposto di `lettura (usage)` -- mai aggiunta finche' non serviva: la sua assenza teneva `cache_scrittura` in italiano nella firma pubblica di `usage/`, accanto a `cache_read` gia' inglese, i due lati della stessa coppia in due lingue | write | ✓ arriva |
...
| verifica (memory) | il metodo di `Indice`/`Lookup` che controlla se l'identificatore che il modello ha proposto esiste davvero nell'anagrafe, con quel tipo (`Indice.verifica`, ora `Lookup.verify`) -- un VERBO (un'azione: "verifica che..."), non il sostantivo che descrive il modulo della riga sopra. Scoperto rinominando `memory/` (Task 5, review): applicare alla cieca `verification` avrebbe prodotto `lookup.verification(tipo, riferimento)`, grammaticalmente sbagliato per un metodo che si chiama come un imperativo | verify | ✓ arriva |
...
> una sola casa). `memory/store.py` usa `ancora` per il legame, di tipo dichiarato, fra un
...
> di classi che entra comunque»): il rischio e' stato evitato, `baseline` restava solo di
> `pavimento` -- e dall'11/09/2026 non e' piu' di nessuno, perche' `mind/baseline.py` e' stato
> cancellato con la fetta «i tre attori».
...
> `intento` (`azione/construction/advisor.py:20-32`, il dizionario con `innesco`/`passi`/`stati`/
...
> `action/registry.py:110`), ma restano due voci distinte apposta.** L'estrazione ha spezzato il
...
| forma | form |
| formatta | format |
| forza (static) | modality | lo stesso concetto di `forza (memory)`, letto dal frontend: `FORZA_OPZIONI` porta preferenza/divieto/fatto/regola. Il campo `forza` sul filo non cambia qui -- lo decide la fetta delle rotte -- cambia solo l'identificatore che lo maneggia (`selForza`) |
| frase | phrase |
| frasi | sentences |
| fresco | fresh |
...
| istantanea | snapshot |
| istante | instant |
| italiano (static) | italian | **Qualificata non perche' abbia due sensi, ma perche' la parola nuda rompe due lettori.** `| italiano | italian |` ha la prima cella uguale all'INTESTAZIONE della tabella: il controllo di completezza n. 1 la scambia per un'intestazione e smette di leggere tutto cio' che segue (misurato: 60 righe sparite in silenzio), e `rinomina.leggi_glossario` la salta di proposito (`if it in ("italiano", "costante")`), quindi la riga sarebbe stata MORTA anche nello strumento. Con la qualificazione la prima cella non e' piu' quella parola e tutt'e due i lettori la vedono. Sta in `nomiRegistriInItaliano` (`tree-route.js:114`, `dashboard.js:235`) |
| lacuna | gap |
| lacune | gaps |
| larghezza | width |
| lato | side |
...
| lenta | slow |
| lettore | reader |
| lettura (static) | read | `build-check.js::letturaLocale()` legge l'identificativo della build locale: l'ATTO di leggere, come `lettura (usage)` |
| limite | limit |
...
| rotta (proxy) | route | **La riga che sarebbe costata piu' cara di tutta la fetta, e non perche' lo strumento sbagli: perche' `rotta` e' un OMOGRAFO vero dell'italiano** -- participio di «rompere» e sostantivo «percorso». Su `proxy/ha_client.py:423` lo strumento proponeva `_rotta_config -> broken_config` (il nome vero, dal lotto 18 di `proxy/`, e' `_config_route`: qui resta il nome di allora perche' e' la misura che ha motivato questa riga), e quel nome **direbbe l'opposto del vero**: quella funzione compone l'URL della rotta di configurazione, e lo dice il suo stesso docstring («L'URL della rotta di configurazione, oppure il motivo del rifiuto»). Non e' un suggerimento storpiato che salta all'occhio come `metti_da_part_l_store_unreadable`: e' **plausibile**, e sarebbe passato in review. **Misurato su tutti gli identificatori del repo che contengono il pezzo `rotta`**: uno in `hiris/app` (`_rotta_config`, senso PERCORSO) e circa 130 nei test (`ROTTA`, `rotta`, `rotta_senza_archivi`, `_Rotta`, tutti senso PERCORSO) contro **UNO** nel senso participio (`porta_rotta`, `tests/test_action_actuator.py:986`). La riga nuda `broken` descrive quindi il senso MINORITARIO, ed e' il verso in cui un'applicazione cieca fa piu' danno. Qualificando `(proxy)`, la riga nuda si spegne per ogni altro ambito (vedi «Il limite della qualificazione per ambito»): e' voluto -- meglio non rinominare che rinominare col senso sbagliato -- e il giorno in cui si convertira' `tests/` va qualificata anche li', guardando ogni occorrenza |
| rotta (api) | route | **Lo stesso senso di `rotta (proxy)`, dichiarato dove serve.** `api/` non e' un ambito convertito, quindi nessuna rinomina automatica parte da qui: questa riga esiste perche' il cancello «ogni parola qualificata e' muta solo dove e' dichiarato» chiede una DECISIONE, e la decisione e' che in `api/` `rotta` significa PERCORSO e mai «guasta». Misurato il 22/09 su tutti gli identificatori di `api/` che portano il pezzo: `ROTTA_APERTA` (`handlers_servizi.py`), il percorso che il confine esenta mentre la finestra di accoppiamento e' aperta — senso percorso, come ogni altro. Senza questa riga il dry-run resterebbe cieco proprio sul codice nuovo |
| salva | save |
...
| sconosciuto | unknown |
| sorta | kind | di che genere e' una riga del `primo piano`: `guasto`, `avviso`, `da_sapere_subito`. **Scelta perche' le tre parole ovvie erano prese**, ciascuna con un altro significato in questo stesso prodotto: `specie` (un impegno e' un fare o un chiedere), `famiglia` (la causa del rifiuto di un provider), `natura` (di chi ha risposto a un turno). Un quarto omonimo qualificato sarebbe costato a ogni lettore una domanda in piu' -- «quale dei quattro?» |
| scritto | written |
| scrivi | write |
| scrivibile | writable |
| secondo | second |
...
| vai | go |
| valida | validate |
| valido | valid | **Il femminile `valida` NON e' aliasato, ed e' una decisione**: `valida` e' il VERBO «convalida» (`validate_config`, in `proxy/ha_client.py` e `azione/construction/workshop.py`), non l'aggettivo. Un alias `valida -> valido` trasformerebbe un metodo pubblico in un aggettivo. Stessa forma della nota su `prima` sotto `primo` |
| valore | value |
| vecchio | old |
| verbo | verb |
...
> davvero il codice: `memory/lookup_cache.py:122` (`self._voci: dict[str, tuple[tuple, Indice]]
...
> invece un concetto diverso, `action/registry.py`: *«lo specchio aggiornato di cosa Home Assistant
...
(`action/`).** `Journal.log`/`Journal.log_construction` (`action/journal.py`) traducevano gia'
...
- `mappa` (`_area_per_entity`, un semplice `entity_id -> nome area`) -> **`area_lookup`**, non
  ...
  `scripts/rinomina.py`). Non e' la stessa cosa del concetto `indice -> lookup` di `memory/`: qui
...
- Composti auto-suggeriti dallo strumento e ACCETTATI cosi' come sono (ordine gia' naturale in
  ...
  vocabolario delle ancore e' di `memory/`, che ha gia' deciso `ancora -> tether`),
...
- `esito` (`handlers_agenda.py`/`handlers_constructions.py`, il dict `{"errore": ...}`/`{...}`
  ...
  della riga parla di "un tentativo" in un senso che in `action/actuator.py` e
  `azione/construction/workshop.py` (gia' chiusi) e' esattamente questo stesso idioma -- non una
...
- `store.scadi(...)` (`handlers_constructions.py`, verso `ConstructionStore.scadi`,
  `azione/construction/revisions.py:295`) **lasciato intatto di proposito**: `scadi` e' un metodo
  PUBBLICO ancora italiano in un ambito gia' chiuso (`action/`), mai deciso nel glossario --
...
- **L'involucro dello strumento `agenda` si chiama ancora `promesse`** (`home_space/tools.py::
  ...
  **involucri di rotta** passano all'inglese: `/api/agenda` emette `{"agenda": [...]}`. Questo qui
  ...
  quattordici, non uno. Pinzato oggi da `tests/test_agenda_api.py`, il cui docstring porta la
  stessa distinzione.
...
- **`sezioni`/`totali` (`handlers_usage.py`, verso `UsageStore.sezioni`/`.totali`), `storia`
  ...
  di `ingresso`: sono QUATTRO metodi PUBBLICI di un ambito gia' chiuso (`usage/`) mai tradotti
...
- **Effetto collaterale reale, chiuso subito**: `carattere -> character` (decisa qui, per
  `MAX_CARATTERI_PROMPT`) si applica anche a `memory/resolver.py` (2 occorrenze, un parametro e
  ...
  verde. Nessun altro pezzo di `memory/resolver.py` toccato.
...
- **Effetto collaterale reale, chiuso subito**: `definizione -> definition` si applica anche a
  `action/verification.py` (6 occorrenze: i parametri di `_declare_target`/`_allows_empty_target` e
  ...
  chiusi: `azione` compariva fra i cambiati insieme al residuo noto `construction/composer.py`.
...
- **Residui dichiarati, con la loro ragione**: `_STORE_DEFAULTS` (importato da
  ...
  (`tests/test_keeper_wiring.py:315`) -- i nomi dei test sono prosa italiana in tutti e 172
...
- **Cio' che resta italiano, e non e' un residuo**: ogni parola chiave verso
  ...
  **Chiuso il 01/09** convertendo `decisione_modelli.py`: tutte e tredici le parole chiave e
  tutti e tre i nomi importati sono passati all'inglese, e i chiamanti li ha trovati il
  controllo di chiusura (313 siti). La promessa scritta poche righe sopra --
  «`scadenza_ponte_min` ... uscira' quando si convertira' quel modulo di radice» -- e'
  mantenuta: `bridge_deadline_min`, col modificatore prima della testa. Questa voce resta
  scritta perche' e' il verbale di cio' che il lotto 13 sapeva e non poteva fare, non un
  elenco di cio' che c'e' oggi.
...
- **Il nome di un test NON si rinomina**:
  `tests/test_knowledge_tools.py:408::test_senza_inventario_leggibile_lo_stato_si_dichiara_non_letto`
...
- ~~`spazio` persistita nell'indice della memoria~~ -- **falso: non c'e' nessuna colonna
  persistita.** `CacheIndice` (`memory/lookup_cache.py:112`, `self._voci: dict[str, tuple[tuple,
  ...
  (`memory/lookup_cache.py:27,65,175,179`), e i suoi VALORI capitano a essere due dei tredici nomi
...
- ~~i nomi vivono in un database, nel `chiamata_json` delle promesse~~ -- **falso: quel campo non
  ...
  a `action/actuator.py:607` (`async def esegui(self, chiamata: dict, *, origine: str)`) e verificata
  contro il registro dei servizi a `action/actuator.py:634` (`verifica(chiamata, self._registro,
...
> Python che gia' implementa l'azione**, `azione/construction/workshop.py:132`
...
>
> **Dubbio chiuso durante la review indipendente del Task 7 (`action/`): il metodo interno dietro
...
> `Workshop.applica` (`azione/construction/workshop.py:328`) e' ora `Workshop.apply`. Stessa
...
> **15 -> 16: `calendar` (fetta «i calendari», Task 3), registrato qui perche' questa e' dove i
> nomi degli strumenti si decidono -- e non passato dalla tabella qui sopra, per la stessa
> ragione di `system_log`/`automation_trace`: quella tabella e' la traduzione ITALIANO ->
> INGLESE dei tredici del 02/09, e `calendar` e' nato gia' in inglese, senza un nome italiano da
> tradurre.
>
> **Perche' `calendar` e non `agenda`.** `agenda` e' gia' preso -- e' l'inglese di `promesse`
> (vedi la tabella qui sopra: «gli impegni futuri che HIRIS tiene in carico»), un concetto
> completamente diverso (le promesse dello schedulatore, non i calendari di Home Assistant).
> Usarlo due volte per due cose diverse sarebbe esattamente la collisione che questa sezione
> esiste per prevenire, non un caso limite: un lettore che vede `agenda` restituire le promesse e
> `calendar` restituire gli impegni di calendario capisce subito che sono due cose; un lettore
> che vedesse `agenda` fare le due cose insieme, o due strumenti chiamati entrambi «agenda» in
> lingue diverse, no.
>
> **Nessuna radice condivisa con nessuno dei quindici precedenti** (`search`, `view`, `related`,
> `remember`, `fetch`, `execute`, `promise`, `agenda`, `cancel`, `propose`, `confirm`, `trend`,
> `logbook`, `system_log`, `automation_trace`): `calendar` non comincia ne' finisce come nessuno
> di questi, ed e' una parola sola, non composta -- diversa anche dalla forma di `system_log`/
> `automation_trace` (dominio_oggetto). E' il nome che Home Assistant stesso usa per il dominio
> delle entita' che questo strumento legge (`calendar.*`) e per la propria API
...
`mind/store.py:91` e `action/journal.py:65`, `specie TEXT NOT NULL` in
...
|---|---|---|---|
| `GENRES` (ex `GENERI`, rinominata dal Task 6 -- solo il nome della costante, non i valori) | funzionamento · presenza · energia · guasto · sicurezza · bilancio | `mind/facts.py:44`; colonna `genere` in `mind/store.py:91` e `action/journal.py:65` |  |
| `ASPECTS` (ex `GAMBE`, rinominata dal Task 6 -- solo il nome della costante, non i valori) | chi c'e' · comfort · dispersione · energia · buono stato · sicurezza | `home_space/type_vocabulary.py` — i nomi delle sei gambe. Vivevano in `mind/baseline.py`, cancellato l'11/09/2026 col pavimento: la gamba non e' piu' un filtro, e' rimasta solo l'ingresso di `facts.genre_for`, e uscira' con i sei generi |  |
...
| `STATES_SOSPESO` (ex `STATI_SOSPESO`, rinominata dal Task 7 (`action/`), gia' applicata a `keeper/promise.py` durante la sua conversione -- solo il nome della costante, non i valori; stessa cautela sulla composizione della riga sopra) | in_attesa · in_corso | `azione/construction/revisions.py:36` e `keeper/promise.py:34` — definita due volte, stesso valore |  |
...
di `action/actuator.py`. Senza, sarebbero le uniche tre su diciassette a scrivere
...
- **omonimi per ambito** (stessa parola italiana, due ambiti): `tether` (`ancora (api)`/
  `ancora (memory)`), `row` (`riga`/`riga (api)`) -- la stessa famiglia di `reference`;
...
**Aggiornato durante la review indipendente del Task 7 (`action/`): l'ottavo caso e' arrivato, ed
