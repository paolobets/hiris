...
| riga (static) | line | **Quinta** qualificazione della stessa parola, e **`static/` porta ENTRAMBI i sensi**, come `ancora`. `line` e' il senso di `albero-route.js::line(parent, text, style)`, che AGGIUNGE UNA RIGA DI TESTO a un nodo -- quello di `riga (casa)` e `riga (agent)`. Ma quattro nomi della stessa cartella portano il senso RECORD, cioe' `riga (api) -> row`: `config/api.js::righe` (un NodeList di `.usage-row`), `models-route.js::rigaProvider` (una riga della tabella della catena, classe CSS `riga-su`/`riga-giu`), `promesse-route.js::costruisciRigaSospeso` e `usage-route.js::rigaModello`. Sono stati rinominati A MANO in `rows`/`providerRow`/`buildPendingRow`/`modelRow`, e questa riga non li copre: e' il limite descritto in «Il limite della qualificazione per ambito», e in `static/` ha due parole invece di una. Chi incontra `riga` qui guarda cosa costruisce prima di fidarsi. `albero-route.js::riga(padre, testo, stile)` AGGIUNGE UNA RIGA DI TESTO a un nodo: e' il senso di `riga (casa)` e `riga (agent)`, non il record di `(api)`/`(proxy)` |
| rileggi | reread |
...
| risultato | result |
| ritardo | delay |
| rivelatore | disclosure | **Corretta il 02/09 usandola.** La fetta di vocabolario aveva scritto `detector`, leggendo la parola e non il codice. Le due sole occorrenze (`costruzioni-route.js:317`, `osservatore-route.js:58`) sono un bottone con `aria-expanded` e `aria-controls` che scopre un pannello: e' il pattern ARIA che si chiama **disclosure**, non un rilevatore di niente. «La prova vera di un nome e' usarlo», e questa e' la prima riga della fetta del frontend che l'uso ha smentito |
| rivendica | claim |
