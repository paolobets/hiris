...
| esito | il fatto osservabile su cio' che e' davvero successo in un tentativo -- un provider che ha rifiutato, un comando riuscito o fallito, un tempo di attesa misurato -- mai un'ipotesi sul perche' -- **quinto caso della famiglia «due sensi dentro lo stesso ambito»** (01/09, `server.py`): `occurrence` e' giusto per l'esito di un PROVIDER, che e' il concetto qui definito e vive in `esiti_provider.py`/`decisione_modelli.py`, cioe' nella stessa radice. Ma `server.py` usa `esito` due volte per il RISULTATO di una lettura -- il payload dei problemi di Home Assistant e il rapporto di `compare_with_home_assistant` -- e li' `occurrence` direbbe una cosa che quel dizionario non e'. Qualificare per ambito non separa niente, perche' i due sensi vivono nello stesso: si decide occorrenza per occorrenza guardando il codice, come per `coda`/`fuori (casa)`. Le due locali di `server.py` sono diventate `report` | occurrence | ✓ arriva |
...
| grezzo (cervello) | un cambiamento di stato registrato esattamente come Home Assistant lo riporta, con le classi che lo accompagnano, prima che qualunque giudizio lo trasformi in un fatto interpretato -- **qualificata l'01/09 per la stessa ragione di `forza (memoria)`, e con una misura in piu': l'inglese di questa riga non e' MAI stato applicato.** Nessun file del prodotto porta `reading` con questo senso, mentre ogni `grezzo` vivo nel codice vuol dire l'aggettivo ordinario «non lavorato»: `server.py:889` (`grezzo = await ha_client.related(...)`, la risposta cruda di un'API) e i quattro di `scripts/rinomina.py::_leggi_grezzo` (il sorgente coi suoi fine-riga). Una riga di una parola sola si applica da sola: lasciata nuda avrebbe scritto `reading` su una risposta HTTP, che e' falso e sembra giusto. Il concetto resta deciso per `cervello/`, dove nascera'; altrove `grezzo` e' `raw`, deciso leggendo | reading | ~ parziale |
